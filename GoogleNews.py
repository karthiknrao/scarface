...
class GoogleNews():
    def __init__( self, qry ):
        self.base = 'https://www.google.co.in/search?tbm=nws&q=%s&start=%d'
        self.qry = '+'.join(qry)
    ...
    def _parse( self, data ):
        ...
        x = soup.findAll( 'a', 'l _HId' )
        y = soup.findAll( 'a', '_sQb' )
        return list(set([ a['href'] for a in x ] + [ a['href'] for a in y ]))
...
if __name__ == '__main__':
    gnews = GoogleNews( [ 'arvind', 'kejriwal' ] )
    gnews.fetch( 5 )
    print gnews.parse(), len(gnews.parse())
